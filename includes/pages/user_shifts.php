...
function load_locations(bool $onlyWithActiveShifts = false)
{
    ...
    if ($onlyWithActiveShifts) {
        $locationIdsFromVolunteerType = NeededVolunteerType::query()
            ->whereNotNull('location_id')
            ->select('location_id');

        $locationIdsFromShift = Shift::query()
            ->leftJoin('needed_volunteer_types', 'shifts.id', 'needed_volunteer_types.shift_id')
            ->leftJoin('needed_volunteer_types AS nast', 'shifts.shift_type_id', 'nast.shift_type_id')
            ->whereNotNull('needed_volunteer_types.id')
            ->orWhereNotNull('nast.id')
            ->select('shifts.location_id');

        $locations->whereIn('id', $locationIdsFromVolunteerType)
...
function load_types()
{
    ...
    if (!VolunteerType::count()) {
        error(__('The administration has not configured any volunteer types yet - or you are not subscribed to any volunteer type.'));
        throw_redirect(url('/'));
    }
    ...
    $types = Db::select(
        '
            SELECT
                ...
                (
                    `volunteer_types`.`restricted`=0
                    OR (
                        NOT `user_volunteer_type`.`confirm_user_id` IS NULL
                        OR `user_volunteer_type`.`id` IS NULL
                    )
                ) AS `enabled`,
                (
                    `user_volunteer_type`.`id` IS NOT NULL
                    AND (
                        `volunteer_types`.`restricted`=0
                        OR `user_volunteer_type`.`confirm_user_id` IS NOT NULL
                    )
                ) AS `own`
            FROM `volunteer_types`
            LEFT JOIN `user_volunteer_type`
                ON (
                    `user_volunteer_type`.`volunteer_type_id`=`volunteer_types`.`id`
                    AND `user_volunteer_type`.`user_id`=?
                )'
            . ($isShico ? '' :
            'WHERE volunteer_types.hide_on_shift_view = 0
                OR user_volunteer_type.user_id IS NOT NULL ') .
            'ORDER BY `own` DESC, `volunteer_types`.`name`
        ',
        [
            $user->id,
        ]
    ...
    if (empty($types)) {
        return unrestricted_volunteertypes();
    }
 ...
 * @return array[]
 */
function unrestricted_volunteertypes()
{
    return VolunteerType::whereRestricted(0)->get(['id', 'name'])->toArray();
}
...
function view_user_shifts()
{
    ...
    /** @var EloquentCollection|UserVolunteerType[] $ownVolunteerTypes */
    $ownVolunteerTypes = UserVolunteerType::whereUserId($user->id)
        ->leftJoin('volunteer_types', 'user_volunteer_type.volunteer_type_id', 'volunteer_types.id')
        ->where(function (Builder $query) {
            $query->whereNotNull('user_volunteer_type.confirm_user_id')
                ->orWhere('volunteer_types.restricted', false);
        })
        ->pluck('volunteer_type_id')->toArray();

    if (empty($ownVolunteerTypes)) {
        // Show at least some shifts even if the user is not in the needed volunteertype / confirmed for it
        $ownVolunteerTypes = UserVolunteerType::whereUserId($user->id)->pluck('volunteer_type_id')->toArray()
            ?: VolunteerType::whereRestricted(false)->pluck('id')->toArray()
            ?: VolunteerType::pluck('id')->toArray();
    }

    if (!$session->has('shifts-filter')) {
        $location_ids = $locations->pluck('id')->toArray();
        $shiftsFilter = new ShiftsFilter(auth()->can('user_shifts_admin'), $location_ids, $ownVolunteerTypes);
        ...
        div('col-md-12', [
            view(__DIR__ . '/../../resources/views/pages/user-shifts.html', [
                ...
                'type_select'   => make_select(
                    ...
                    'types',
                    icon('person-lines-fill') . __('volunteertypes.volunteertypes') . ' '
                    . '<a class="icon-link" href="' . url('/volunteertypes/about') . '" title="' . __('volunteertypes.about') . '" target="_blank">'
...
function make_select($items, $selected, $name, $title = null, $ownSelect = [])
{
    ...
    foreach ($items as $i => $item) {
        ...
        $htmlItems[] = '<div class="form-check">'
            ...
            . '</div>'
            . ($break ? '<hr class="border border-info border-2 opacity-75" id="volunteer_types_selection_hr">' : '');
    }
    $html .= implode("\n", $htmlItems);
