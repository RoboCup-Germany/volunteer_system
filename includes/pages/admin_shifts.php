...
function admin_shifts()
{
    ...
    // Load volunteertypes
    /** @var VolunteerType[]|Collection $types */
    $types = VolunteerType::all();
    $no_volunteertypes = $types->isEmpty();
    $needed_volunteer_types = [];
    foreach ($types as $type) {
        $needed_volunteer_types[$type->id] = 0;
    }
    ...
    if ($request->has('preview') || $request->has('back')) {
        ...
        if ($request->has('volunteermode')) {
            if ($request->input('volunteermode') == 'shift_type') {
                $volunteermode = 'shift_type';
            } elseif ($request->input('volunteermode') == 'location') {
                $volunteermode = 'location';
            } elseif ($request->input('volunteermode') == 'manually') {
                foreach ($types as $type) {
                    if (preg_match('/^\d+$/', trim($request->input('volunteertype_count_' . $type->id, 0)))) {
                        $needed_volunteer_types[$type->id] = trim($request->input('volunteertype_count_' . $type->id, 0));
                    } else {
                        $valid = false;
                        error(sprintf(__('Please check the needed volunteers for team %s.'), $type->name));
                    }
                }
            } else {
                $valid = false;
                error(__('Please select a mode for needed volunteers.'));
            }

            if (
                $volunteermode == 'manually' && array_sum($needed_volunteer_types) == 0
                || $volunteermode == 'location' && !NeededVolunteerType::whereLocationId($lid)
                    ->where('count', '>', '0')
                    ->count()
                || $volunteermode == 'shift_type' && !NeededVolunteerType::whereShiftTypeId($shifttype_id)
                    ->where('count', '>', '0')
                    ->count()
            ) {
                $valid = false;
                error(__('There are 0 volunteers needed. Please enter the amounts of needed volunteers.'));
            }
        } else {
            $valid = false;
            error(__('Please select needed volunteers.'));
        }
        ...
        // All entries OK
        if ($valid) {
            if ($volunteermode == 'shift_type') {
                $needed_volunteer_types = NeededVolunteerType::whereShiftTypeId($shifttype_id)
                        ->pluck('count', 'volunteer_type_id')
                        ->toArray() + $needed_volunteer_types;
            } elseif ($volunteermode == 'location') {
                $needed_volunteer_types = NeededVolunteerType::whereLocationId($lid)
                        ->pluck('count', 'volunteer_type_id')
                        ->toArray() + $needed_volunteer_types;
            }
            ...
            foreach ($shifts as $shift) {
                ...
                foreach ($types as $type) {
                    if (isset($needed_volunteer_types[$type->id]) && $needed_volunteer_types[$type->id] > 0) {
                        $shifts_table_entry['needed_volunteers'] .= '<b>' . VolunteerType_name_render($type) . ':</b> '
                            . $needed_volunteer_types[$type->id] . '<br />';
                    }
                }
            ...
            $session->set('admin_shifts_types', $needed_volunteer_types);

            $previousEntries = [];
            foreach ($needed_volunteer_types as $type_id => $count) {
                $previousEntries['volunteertype_count_' . $type_id] = $count;
            }
            ...
            return page_with_title(__('form.preview'), [
                form([
                    ...
                    table([
                        ...
                        'needed_volunteers' => __('Needed volunteers'),
                    ], $shifts_table),
        ...
        foreach ($session->get('admin_shifts_shifts', []) as $shift) {
            ...
            foreach ($session->get('admin_shifts_types', []) as $type_id => $count) {
                $volunteer_type_source = VolunteerType::find($type_id);
                if (!empty($volunteer_type_source) && $count > 0) {
                    $neededVolunteerType = new NeededVolunteerType();
                    $neededVolunteerType->shift()->associate($shift);
                    $neededVolunteerType->volunteerType()->associate($volunteer_type_source);
                    $neededVolunteerType->count = $count;
                    $neededVolunteerType->save();

                    $needed_volunteer_types_info[] = $volunteer_type_source->name . ': ' . $count;
                }
            }
    ...
    $volunteer_types = '';
    foreach ($types as $type) {
        $volunteer_types .= '<div class="col-sm-6 col-md-8 col-lg-6 col-xl-4 col-xxl-3">'
            . form_spinner(
                'volunteertype_count_' . $type->id,
                htmlspecialchars($type->name),
                $needed_volunteer_types[$type->id],
                [
                    'radio-name'  => 'volunteermode',
    ...
    return page_with_title(
        ...
        [
            ...
            $no_volunteertypes ? warning(__('admin_shifts.no_volunteertypes')) : '',
            msg(),
            form([
                div('row', [
                ...
                div('row', [
                    ...
                    div('col-md-6 col-xl-7', [
                        form_info(__('Needed volunteers')),
                        form_radio(
                            'volunteermode',
                            __('Copy needed volunteers from shift type settings'),
                            $volunteermode == 'shift_type',
                            'shift_type'
                        ),
                        form_radio(
                            'volunteermode',
                            __('Copy needed volunteers from location settings'),
                            $volunteermode == 'location',
                            'location'
                        ),
                        form_radio(
                            'volunteermode',
                            __('The following volunteers are needed'),
                            $volunteermode == 'manually',
                            'manually'
                        ),
                        div('row', [
