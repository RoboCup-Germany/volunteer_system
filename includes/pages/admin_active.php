...
function admin_active()
{
    ...
    if ($request->has('set_active')) {
        if ($request->has('count') && preg_match('/^\d+$/', $request->input('count'))) {
            ...
            if ($count < $forced_count && config('enable_force_active')) {
                error(sprintf(
                    __('At least %s volunteers are forced to be active. The number has to be greater.'),
                    $forced_count
                ));
                throw_redirect(url('/admin-active'));
            }
        } else {
            $msg .= error(__('Please enter a number of volunteers to be marked as active.'));
            throw_redirect(url('/admin-active'));
        }
        ...
        if ($request->hasPostData('ack')) {
            ...
            volunteersystem_log('These volunteers are active now: ' . join(', ', $user_nicks));

            $msg = success(__('Marked volunteers.'), true);
        } else {
            $set_active = form([
    ...
    if ($request->hasPostData('submit')) {
        if ($request->has('active') && preg_match('/^\d+$/', $request->input('active'))) {
            ...
            if ($user_source) {
                ...
                volunteersystem_log('User ' . User_Nick_render($user_source, true) . ' is active now.');
                $msg = success(__('Volunteer has been marked as active.'), true);
            } else {
                $msg = error(__('Volunteer not found.'), true);
            }
            ...
            if ($user_source) {
                ...
                $user_source->state->save();
                volunteersystem_log('User ' . User_Nick_render($user_source, true) . ' is NOT active now.');
                $msg = success(__('Volunteer has been marked as not active.'), true);
            } else {
                $msg = error(__('Volunteer not found.'), true);
            }
            ...
            if ($user_source) {
                if (
                    ...
                    $msg = error(__('Volunteer has no valid T-shirt size. T-shirt was not set.'), true);
                } else {
                    $user_source->state->got_goodie = true;
                    $user_source->state->save();
                    volunteersystem_log('User ' . User_Nick_render($user_source, true) . ' has goodie now.');
                    $msg = success(
                        __('Volunteer got a goodie.'),
                        true
                    );
                }
            } else {
                $msg = error('Volunteer not found.', true);
            }
            ...
            if ($user_source) {
                ...
                $user_source->state->save();
                volunteersystem_log('User ' . User_Nick_render($user_source, true) . ' has NO goodie.');
                $msg = success(
                    __('Volunteer got no goodie.'),
                    true
                );
            } else {
                $msg = error(__('Volunteer not found.'), true);
    ...
    foreach ($users as $user) {
        ...
        if ($goodie_tshirt) {
            $userData['shirt_size'] = (isset($tshirt_sizes[$shirtSize]) ? $tshirt_sizes[$shirtSize]
                ...
                . __("This Volunteer has no valid T-shirt size, therefore it's not possible to hand out a T-shirt.")
                . '"></span></small>'
            );
        }
    ...
    return page_with_title(admin_active_title(), [
        form([
            form_text('search', __('Search volunteer:'), $search),
            form_checkbox('show_all_shifts', __('Show all shifts'), $show_all_shifts),
            form_submit('submit', icon('search') . __('form.search')),
        ], url('/admin-active')),
        $set_active == '' ? form([
            form_text('count', __('How many volunteers should be active?'), $count ?: $forced_count),
