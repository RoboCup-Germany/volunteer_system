...
function admin_user()
{
    ...
    if (!$request->has('action')) {
        ...
        $html .= __('Here you can change the user entry. Under the item \'Arrived\' the volunteer is marked as present, a yes at Active means that the volunteer was active.');
        if ($goodie_enabled && $user_goodie_edit) {
            $html .= ' ' . __('If the volunteer is active, it can claim a goodie. If goodie is set to \'Yes\', the volunteer already got their goodie.');
        }
        $html .= '<br><br>';
        $html .= '<form action="'
        ...
        $html .= __('Here you can reset the password of this volunteer:');
        ...
        $volunteer_highest_group = $user_source->groups()->orderByDesc('id')->first();
        if (!empty($volunteer_highest_group)) {
            $volunteer_highest_group = $volunteer_highest_group->id;
        }

        if (
            ($user_id != $user->id || auth()->can('admin_groups'))
            && ($my_highest_group >= $volunteer_highest_group || is_null($volunteer_highest_group))
        ) {
            $html .= __('Here you can define the user groups of the volunteer:') . '<form action="'
        ...
        switch ($request->input('action')) {
            case 'save_groups':
                $volunteer = User::findOrFail($user_id);
                if ($volunteer->id != $user->id || auth()->can('admin_groups')) {
                    /** @var Group $my_highest_group */
                    $my_highest_group = $user->groups()->orderByDesc('id')->first();
                    /** @var Group $volunteer_highest_group */
                    $volunteer_highest_group = $volunteer->groups()->orderByDesc('id')->first();

                    if (
                        $my_highest_group
                        && (
                            empty($volunteer_highest_group)
                            || ($my_highest_group->id >= $volunteer_highest_group->id)
                        )
                    ) {
                        $groups_source = changeableGroups($my_highest_group->id, $volunteer->id);
                        $groups = [];
                        $groupList = [];
                        ...
                        $volunteer->groups()->detach();
                        $user_groups_info = [];
                        foreach ($groupsRequest as $group) {
                            if (in_array($group, $groupList)) {
                                $group = $groups[$group];
                                $volunteer->groups()->attach($group);
                                $user_groups_info[] = $group->name;
                            }
                        }
                        volunteersystem_log(
                            'Set groups of ' . User_Nick_render($volunteer, true) . ' to: '
                            . join(', ', $user_groups_info)
                        );
                        $html .= success(__('User groups saved.'), true);
                    } else {
                        $html .= error(__('You cannot edit volunteers with more rights.'), true);
                    }
                } else {
            ...
            case 'change_pw':
                if (
                    ...
                    volunteersystem_log('Set new password for ' . User_Nick_render($user_source, true));
 ...
 */
function changeableGroups($myHighestGroup, $volunteerId): Collection
{
    return Group::query()
        ->where('groups.id', '<=', $myHighestGroup)
        ->join('users_groups', function ($query) use ($volunteerId) {
            /** @var JoinClause $query */
            $query->where('users_groups.group_id', '=', $query->raw('groups.id'))
                ->where('users_groups.user_id', $volunteerId);
