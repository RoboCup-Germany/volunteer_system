...
function user_myshifts()
{
    $user = auth()->user();
    $request = request();
    $is_volunteertype_supporter = false;
    if ($request->has('edit')) {
        $id = $request->input('edit');
        $shiftEntry = ShiftEntry::where('id', $id)
            ->where('user_id', User::find($request->input('id'))?->id)
            ->first();
        $is_volunteertype_supporter = $shiftEntry && auth()->user()->isVolunteerTypeSupporter($shiftEntry->volunteerType);
    }

    if (
        $request->has('id')
        && (auth()->can('user_shifts_admin') || $is_volunteertype_supporter)
