...
use Volunteersystem\Helpers\UserVouchers;
...
/**
 * @param array $needed_volunteer_type
 * @return string
 */
function User_view_shiftentries($needed_volunteer_type)
{
    $shift_info = '<br><b><a href="'
        . url('/volunteertypes', ['action' => 'view', 'volunteertype_id' => $needed_volunteer_type['id']])
        . '">' . htmlspecialchars($needed_volunteer_type['name']) . '</a>:</b> ';

    $shift_entries = [];
    foreach ($needed_volunteer_type['users'] as $user_shift) {
...
function User_view_myshift(Shift $shift, $user_source, $its_me, $supporter)
{
    ...
    $goodie = GoodieType::from(config('goodie_type'));
    $goodie_enabled = $goodie !== GoodieType::None;
    $supporter = auth()->user()->isVolunteerTypeSupporter(VolunteerType::findOrFail($shift->volunteer_type_id));
    ...
    foreach ($shift->needed_volunteertypes as $needed_volunteer_type) {
        $shift_info .= User_view_shiftentries($needed_volunteer_type);
    }
    ...
    if (Shift_signout_allowed($shift, (new VolunteerType())->forceFill(['id' => $shift->volunteer_type_id]), $user_source->id)) {
        $myshift['actions'][] = button(
 ...
 * @param Collection|int[] $supported_volunteertypes
 *
 * @return array
 */
function User_view_myshifts(
    ...
    $goodie = GoodieType::from(config('goodie_type'));
    $goodie_enabled = $goodie !== GoodieType::None;
    $supported_volunteertypes = auth()->user()
        ->userVolunteerTypes()
    ...
    foreach ($shifts as $shift) {
        ...
        $supporter = $supported_volunteertypes->contains($shift->volunteer_type_id);
 ...
 * @param VolunteerType[] $user_volunteertypes
...
function User_view(
    ...
    $supported_volunteertypes = auth()->user()
        ...
        ->where('supporter', true)
        ->pluck('volunteer_types.id');
    $user_volunteertypes_supporter = false;
    foreach ($user_source->userVolunteerTypes as $user_volunteertype) {
        if ($supported_volunteertypes->contains($user_volunteertype->id)) {
            $user_volunteertypes_supporter = true;
            break;
        }
    }

    if ($its_me || $admin_user_privilege || $goodie_admin || $user_volunteertypes_supporter) {
    ...
    foreach ($user_volunteertypes as $volunteertype) {
        $needs_drivers_license = $needs_drivers_license || $volunteertype->requires_driver_license;
    }

    $needs_ifsg_certificate = false;
    foreach ($user_volunteertypes as $volunteertype) {
        $needs_ifsg_certificate = $needs_ifsg_certificate || $volunteertype->requires_ifsg_certificate;
    }
    ...
    return page_with_title(
        '<span class="icon-icon_volunteer"></span> '
        ...
        [
            ...
            div('row user-info', [
                ...
                User_volunteertypes_render($user_volunteertypes),
 ...
 * @return string
 */
function User_volunteertypes_render($user_volunteertypes)
{
    $output = [];
    foreach ($user_volunteertypes as $volunteertype) {
        $class = 'text-success';
        if ($volunteertype->restricted && !$volunteertype->pivot->confirm_user_id) {
            $class = 'text-warning';
        }
        $output[] = '<a href="' . volunteertype_link($volunteertype->id) . '" class="' . $class . '">'
            . ($volunteertype->pivot->supporter ? icon('patch-check') : '') . htmlspecialchars($volunteertype->name)
            . '</a>';
    }
    return div('col-md-2', [
        heading(__('volunteertypes.volunteertypes'), 4),
        join('<br>', $output),
    ]);
}
...
function User_Nick_render($user, $plain = false)
{
    ...
    return render_profile_link(
        '<span class="icon-icon_volunteer"></span>&nbsp;' . htmlspecialchars($user->displayName) . '</a>',
        $user->id,
...
/**
 * hint for volunteers, which are not arrived yet
 *
 * @return string|null
 */
    ...
    if (config('signup_requires_arrival') && !$user->state->arrived && !$is_before_arrival_date) {
        ...
        if (!empty($buildup) && $buildup->lessThan(new Carbon())) {
            return $user->state->user_info
                ...
                : __('You are not marked as arrived. Please go to heaven, get your volunteer badge and/or tell them that you arrived already.');
        }
    }
