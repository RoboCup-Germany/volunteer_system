 ...
 * @return string HTML
 */
function ShiftEntry_delete_view_admin(Shift $shift, VolunteerType $volunteertype, User $signoff_user)
{
 ...
 * @return string HTML
 */
function ShiftEntry_delete_view(Shift $shift, VolunteerType $volunteertype, User $signoff_user)
{
    return page_with_title(ShiftEntry_delete_title(), [
        info(sprintf(
 ...
 * @param VolunteerType $volunteertype
 * @param array     $volunteertypes_select
...
function ShiftEntry_create_view_admin(
    ...
    return page_with_title(
        ...
        [
            ...
            form([
                form_select('volunteertype_id', __('Volunteer type'), $volunteertypes_select, $volunteertype->id),
 ...
 * @return string
 */
function ShiftEntry_create_view_user(Shift $shift, Location $location, VolunteerType $volunteertype, $comment)
{
    $start = $shift->start->format(__('general.datetime'));
    return page_with_title(
        ShiftEntry_create_title() . ': ' . htmlspecialchars($shift->shiftType->name)
        ...
        [
            Shift_view_header($shift, $location),
            info(sprintf(__('Do you want to sign up for this shift as %s?'), $volunteertype->name), true),
            form([
...
function ShiftEntry_edit_view(
    ...
    $volunteertype_supporter = false
) {
    $freeload_form = [];
    $goodie = GoodieType::from(config('goodie_type'));
    $goodie_enabled = $goodie !== GoodieType::None;

    if ($user_admin_shifts || $volunteertype_supporter) {
        if (!$goodie_enabled) {
    ...
    $link = button(
        url('/users', ['action' => 'view', 'user_id' => $volunteer->id]),
        icon('chevron-left'),
        'btn-sm',
        '',
        __('general.back'),
    ...
    return page_with_title(
        ...
        [
            msg(),
            form([
                form_info(__('Volunteer:'), User_Nick_render($volunteer)),
