...
function location_view(Location $location, ShiftsFilterRenderer $shiftsFilterRenderer, ShiftCalendarRenderer $shiftCalendarRenderer)
{
    ...
    if (auth()->can('admin_shifts') && $location->neededVolunteerTypes->isNotEmpty()) {
        $neededVolunteerTypes .= '<h3>' . __('location.required_volunteers') . '</h3><ul>';
        foreach ($location->neededVolunteerTypes as $neededVolunteerType) {
            if ($neededVolunteerType->count) {
                $neededVolunteerTypes .= '<li><a href="'
                    . url('volunteertypes', ['action' => 'view', 'volunteertype_id' => $neededVolunteerType->volunteerType->id])
                    . '">' . $neededVolunteerType->volunteerType->name
                    . '</a>: '
                    . $neededVolunteerType->count
                    . '</li>';
            }
        }
        $neededVolunteerTypes .= '</ul>';
    }
