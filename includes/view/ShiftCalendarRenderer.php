...
class ShiftCalendarRenderer
{
     ...
     *
     * @param Shift[]                 $shifts
     * @param array[]                 $needed_volunteertypes
     * @param ShiftEntry[][]|Collection $shift_entries
     * @param ShiftsFilter            $shiftsFilter
     */
    public function __construct($shifts, private $needed_volunteertypes, private $shift_entries, ShiftsFilter $shiftsFilter)
    {
    ...
    private function renderLane(ShiftCalendarLane $lane)
    {
        ...
        foreach ($lane->getShifts() as $shift) {
            ...
            $needed_volunteertypes = collect($this->needed_volunteertypes[$shift->id]);

            // Add volunteer types from shift entries without reference from needed volunteer types
            foreach (
                $shift->shiftEntries
                    ->whereNotIn('volunteer_type_id', $needed_volunteertypes->pluck('id'))
                    ->groupBy('volunteer_type_id') as $shiftEntriesOfVolunteerType
            ) {
                /** @var Collection|ShiftEntry[] $shiftEntriesOfVolunteerType */
                /** @var VolunteerType $volunteertype */
                $volunteertype = $shiftEntriesOfVolunteerType->first()->volunteerType;
                $needed_volunteertypes[] = [
                    ...
                    'shift_self_signup' => $volunteertype->shift_self_signup,
                ];
            }
    ...
    private function renderLegend()
    {
        return div('legend mt-3', [
            ...
            badge(__('Other volunteer type needed / collides with my shifts'), 'warning'),
