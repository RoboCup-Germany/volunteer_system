...
/**
 * Returns the number of volunteers currently working.
 *
...
function stats_currently_working(?ShiftsFilter $filter = null): int|string
{
    $result = Db::selectOne(
        ...
        SELECT SUM((
                ...
                ' . ($filter ? 'AND shift_entries.volunteer_type_id IN (' . implode(',', $filter->getTypes()) . ')' : '') . '
            )) AS `count`
        FROM `shifts`
...
function stats_hours_to_work(?ShiftsFilter $filter = null): int|string
{
    $result = Db::selectOne(
        ...
        SELECT ROUND(SUM(`count`)) AS `count` FROM (
            SELECT
                (SELECT SUM(`count`) FROM `needed_volunteer_types` WHERE `needed_volunteer_types`.`shift_id`=`shifts`.`id`' . ($filter ? ' AND needed_volunteer_types.volunteer_type_id IN (' . implode(',', $filter->getTypes()) . ')' : '') . ')
            ...
            /* By shift type */
            SELECT
                (SELECT SUM(`count`) FROM `needed_volunteer_types` WHERE `needed_volunteer_types`.`shift_type_id`=`shifts`.`shift_type_id`' . ($filter ? ' AND needed_volunteer_types.volunteer_type_id IN (' . implode(',', $filter->getTypes()) . ')' : '') . ')
                * TIMESTAMPDIFF(MINUTE, `shifts`.`start`, `shifts`.`end`) / 60 AS `count`
            FROM `shifts`
            LEFT JOIN schedule_shift AS s on shifts.id = s.shift_id
            ...
            /* By location */
            SELECT
                (SELECT SUM(`count`) FROM `needed_volunteer_types` WHERE `needed_volunteer_types`.`location_id`=`shifts`.`location_id`' . ($filter ? ' AND needed_volunteer_types.volunteer_type_id IN (' . implode(',', $filter->getTypes()) . ')' : '') . ')
                * TIMESTAMPDIFF(MINUTE, `shifts`.`start`, `shifts`.`end`) / 60 AS `count`
            FROM `shifts`
            LEFT JOIN schedule_shift AS s on shifts.id = s.shift_id
            LEFT JOIN schedules AS se on s.schedule_id = se.id
...
/**
 * Returns the number of needed volunteers in the next 3 hours
 *
 * @param ShiftsFilter|null $filter
 *
 * @return int|string
 */
function stats_volunteers_needed_three_hours(?ShiftsFilter $filter = null): int|string
{
    ...
    $result = Db::selectOne('
        SELECT SUM(`count`) AS `count` FROM (
            SELECT
                GREATEST(0,
                    (
                    SELECT SUM(needed_volunteer_types.`count`)
                    FROM `needed_volunteer_types`
                    JOIN `volunteer_types` ON `volunteer_types`.`id`=`needed_volunteer_types`.`volunteer_type_id`
                    WHERE `volunteer_types`.`show_on_dashboard`=TRUE
                        AND `needed_volunteer_types`.`shift_id`=`shifts`.`id`
                        ' . ($filter ? 'AND needed_volunteer_types.volunteer_type_id IN (' . implode(',', $filter->getTypes()) . ')' : '') . '
                    ) - (
                    SELECT COUNT(*)
                    FROM `shift_entries`
                    JOIN `volunteer_types` ON `volunteer_types`.`id`=`shift_entries`.`volunteer_type_id`
                    WHERE `volunteer_types`.`show_on_dashboard`=TRUE
            ...
            SELECT
                GREATEST(0,
                    ...
                    JOIN `volunteer_types` ON `volunteer_types`.`id`=`needed_volunteer_types`.`volunteer_type_id`
                    WHERE `volunteer_types`.`show_on_dashboard`=TRUE
                        AND `needed_volunteer_types`.`shift_type_id`=`shifts`.`shift_type_id`
                        ' . ($filter ? 'AND needed_volunteer_types.volunteer_type_id IN (' . implode(',', $filter->getTypes()) . ')' : '') . '
                    ) - (
                    SELECT COUNT(*)
                    FROM `shift_entries`
            ...
            SELECT
                GREATEST(0,
                    ...
                    JOIN `volunteer_types` ON `volunteer_types`.`id`=`needed_volunteer_types`.`volunteer_type_id`
                    WHERE `volunteer_types`.`show_on_dashboard`=TRUE
                        AND `needed_volunteer_types`.`location_id`=`shifts`.`location_id`
                        ' . ($filter ? 'AND needed_volunteer_types.volunteer_type_id IN (' . implode(',', $filter->getTypes()) . ')' : '') . '
                    ) - (
                    SELECT COUNT(*)
                    FROM `shift_entries`
...
/**
 * Returns the number of needed volunteers for nightshifts (see config)
 *
 * @param ShiftsFilter|null $filter
 *
 * @return int|string
 */
function stats_volunteers_needed_for_nightshifts(?ShiftsFilter $filter = null): int|string
{
        ...
        SELECT SUM(`count`) AS `count` FROM (
            ...
            SELECT
                GREATEST(0,
                    ...
                    WHERE `volunteer_types`.`show_on_dashboard`=TRUE
                        AND `needed_volunteer_types`.`shift_type_id`=`shifts`.`shift_type_id`
                        ' . ($filter ? 'AND volunteer_types.id IN (' . implode(',', $filter->getTypes()) . ')' : '') . '
                    ) - (
                    SELECT COUNT(*) FROM `shift_entries`
                    JOIN `volunteer_types` ON `volunteer_types`.`id`=`shift_entries`.`volunteer_type_id`
                    WHERE `volunteer_types`.`show_on_dashboard`=TRUE
