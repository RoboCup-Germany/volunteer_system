...
namespace Volunteersystem\Factories;
...
use Volunteersystem\Models\User\User as VolunteersystemUser;
...
class User
{
     ...
     * @return VolunteersystemUser The user if successful
     * @throws
     */
    public function createFromData(array $rawData): VolunteersystemUser
    {
    ...
    {
        if (VolunteersystemUser::whereName($username)->exists()) {
            throw new ValidationException(
    ...
    {
        if (VolunteersystemUser::whereEmail($email)->exists()) {
            throw new ValidationException(
     ...
     * @param Array<string, mixed> $rawData
     */
    private function createUser(array $data, array $rawData): VolunteersystemUser
    {
        // Ensure all user entries got created before saving
        $this->dbConnection->beginTransaction();

        $user = new VolunteersystemUser([
            'name'          => $data['username'],
            'password'      => '',
            'email'         => $data['email'],
            'api_key'       => '',
        ...
        $assignedVolunteerTypeNames = $this->assignVolunteerTypes($user, $rawData);

        $this->logger->info(
            'User {user} registered and signed up as: {volunteertypes}',
            [
                'user' => sprintf('%s (%u)', $user->displayName, $user->id),
                'volunteertypes' => join(', ', $assignedVolunteerTypeNames),
            ]
        );
    ...
    private function assignVolunteerTypes(VolunteersystemUser $user, array $rawData): array
    {
        $possibleVolunteerTypes = VolunteerType::whereHideRegister(false)->get();
        $assignedVolunteerTypeNames = [];

        foreach ($possibleVolunteerTypes as $possibleVolunteerType) {
            $volunteerTypeCheckboxId = 'volunteer_types_' . $possibleVolunteerType->id;

            if (array_key_exists($volunteerTypeCheckboxId, $rawData)) {
                $user->userVolunteerTypes()->attach($possibleVolunteerType);
                $assignedVolunteerTypeNames[] = $possibleVolunteerType->name;
            }
        }

        return $assignedVolunteerTypeNames;
    }
