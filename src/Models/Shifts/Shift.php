...
class Shift extends BaseModel
{
        ...
        $query
            ->addSelect([
                ...
                'needs_users' => Shift::from('shifts as s2')
                    ...
                    ->leftJoin('needed_volunteer_types', function (JoinClause $join): void {
                        // Directly
                        $join->on('needed_volunteer_types.shift_id', 's2.id')
                            // Via schedule location
                            ->orOn('needed_volunteer_types.location_id', 's2.location_id')
                            // Via schedule shift type
                            ->orOn('needed_volunteer_types.shift_type_id', 'schedules.shift_type');
                    })
                    ...
                    ->where(function (Builder $query): void {
                        $query
                            ->where(function (Builder $query): void {
                                $query
                                    // Direct requirement
                                    ->whereColumn('needed_volunteer_types.shift_id', 's2.id')
                                    // Or has schedule & via location
                                    ->orWhere(function (Builder $query): void {
                                        $query
                                            ->where('schedules.needed_from_shift_type', false)
                                            ->whereColumn('needed_volunteer_types.location_id', 's2.location_id');
                                    })
                                    // Or has schedule & via type
                                    ->orWhere(function (Builder $query): void {
                                        $query
                                            ->where('schedules.needed_from_shift_type', true)
                                            ->whereColumn('needed_volunteer_types.shift_type_id', 's2.shift_type_id');
                                    });
                            });
                    })
                    ->selectRaw('COUNT(*) > 0'),
    ...
    public function isNightShift(): bool
    {
        ...
        /** @see \Volunteersystem\Helpers\Goodie::shiftScoreQuery to keep them in sync */
